...
struct
  ...
  let rec fold (f: key -> value -> 'a -> 'a) (u: 'a) (d: dict) : 'a =
    ...
    | Two (left, (key, value), right) ->
        let acc_left = fold f u left in 
        let acc_kv = f key value acc_left in 
        fold f acc_kv right 
    | Three (left, (key1, val1), middle, (key2, val2), right) ->
        let acc_left = fold f u left in
        let acc_kv1 = f key1 val1 acc_left in
        let acc_middle = fold f acc_kv1 middle in 
        let acc_kv2 = f key2 val2 acc_middle in 
        fold f acc_kv2 right
...
*)
  ...
  let insert_upward_three (w: pair) (w_left: dict) (w_right: dict)
    ...
    assert(match D.compare x_key y_key with | Less -> true | _ -> false) ;
  ...
  and insert_downward_three ((k,v): pair) ((k1,v1): pair) ((k2,v2): pair)
    ...
    | (Less, _) -> 
        (match insert_downward left k v with
         | Up (w_left, w, w_right) -> 
             insert_upward_three w w_left w_right (k1, v1) (k2, v2) middle right
         | Done new_left -> 
             Done (Three (new_left, (k1, v1), middle, (k2, v2), right)))
    | (Greater, Eq) -> Done (Three (left, (k1,v1), middle, (k, v), right))
    | (Greater, Less) ->
        (match insert_downward middle k v with
         | Up (w_left, w, w_right) ->
             insert_upward_three w w_left w_right (k1, v1) (k2, v2) left right
         | Done new_middle -> 
             Done (Three (left, (k1,v1), new_middle, (k2, v2), right)))
    | (Greater, Greater) ->
        (match insert_downward right k v with
         | Up (w_left, w, w_right) -> 
             insert_upward_three w w_left w_right (k1, v1) (k2, v2) left middle
         | Done new_right ->
             Done (Three (left, (k1,v1), middle, (k2,v2), new_right)))
  ...
  let rec lookup (d: dict) (k: key) : value option =
    ...
    | Three (left, (key1, val1), middle, (key2, val2), right) ->
        (match (D.compare k key1, D.compare k key2) with
        | (Eq, _) -> Some val1
        | (Less, _) -> lookup left k
        | (Greater, Eq) -> Some val2
  ...
  let choose (d: dict) : (key * value * dict) option =
    let rec helper (d:dict) : (key * value * dict) option = 
      match d with
      | Leaf -> None
      | Two (l, (k,v),r) ->
          (match helper l with
          | None -> Some (k, v, remove d k)
          | Some (k', v', new_l)  -> Some (k', v', Two (new_l, (k,v),r)))
      | Three (l, (k1,v1), m, (k2, v2), r) ->
          (match helper l with
          | None -> Some (k1, v1, remove d k1)
          | Some (k', v', new_l) -> 
              Some (k', v', Three (new_l,(k1,v1),m,(k2,v2),r)))
    in helper d
  ...
  let rec balanced (d: dict) : bool =
    ...
    let max_of_three a b c = (max (max a b) c) in
    let rec height (d:dict) : int =
      ...
      | Three (left, _, middle, _, right) ->
          1 + (max_of_three (height left) (height middle) (height right)) 
    in 
      match d with
      | Leaf -> true
      | Two (left, _, right) -> 
